...
| 分类 | 功能 |
| --- | --- |
| **外观** | 明暗主题、**24 色中国传统紫配色切换**（本地记忆）、字号 A-/A+ |
...
全站配色只有一个来源：`src/palette.ts` 里的 **24 个中国传统紫色**（淡牵牛紫、凤信紫、青莲、雪青、满天星紫、葡萄青、紫茄……）。读者可在页面右下角的调色按钮里随时切换，选择保存在本地。

- **默认色**：`DEFAULT_ACCENT = '青莲'`（`#7b5aa3`）
- **令牌推导**：`tokensFor()` 由单个色值算出一整套六个变量——亮色主题压暗到 L≤44% 保证浅底对比度，暗色主题提亮到 L≥72% 保证深底可读，不需要为每个颜色手写两套值
- **映射**：`book.css` 只负责把 `--stb-accent*` 映射到 Starlight 的 `--sl-color-accent*`，因此切换配色对 Starlight 组件全局生效
- **无闪烁**：CSS 里写死了默认色对应的六个值，未做选择时不写内联变量，首屏不会闪

> 封面插画与分享卡片是**预生成的位图/SVG**，不随运行时切换改变（面板里已注明）。若要换默认色，改 `DEFAULT_ACCENT` 后重跑：
> `npm run covers && npm run og && npm run brand`
> 注意插画与品牌图必须在本地生成后提交——CI 的 Linux 环境没有中文字体。

## 响应式与触控适配

站点在手机（≤640px）、平板（641–1024px）、桌面三档下分别调优，并对触屏单独处理：

| 关注点 | 处理方式 |
| --- | --- |
| 触控目标 | `@media (pointer: coarse)` 下所有注入控件 ≥40px（字号按钮 / 返回顶部 46px，工具栏 44px） |
| hover 依赖 | 代码下载按钮与术语锚点原本只在 hover 时出现，触屏上等于不存在 → 触屏下改为常显 |
| iPhone 安全区 | 浮动控件与提示条使用 `env(safe-area-inset-bottom)`，避开刘海与底部指示条 |
| 平板字号 | 641–1024px 保持 14.5px（此前被 768px 断点误判为手机，正文掉到 13.5px） |
| 窄屏插图 | 宽矢量插图包进 `.stb-figure-scroll` 横向滚动，而不是压到看不清 |
| 窄屏表格 | ≤640px 转为卡片式，每个单元格带 `data-label` 字段名 |
| 窄屏工具栏 | 章末按钮改为两列铺满；≤380px 单列 |
| 点击反馈 | 取消 `-webkit-tap-highlight-color` 灰块，`touch-action: manipulation` 去掉双击缩放延迟 |
| 术语密度 | 每个术语每页只链首次出现（窄屏上满篇下划线严重影响阅读） |

端到端测试分三个项目：`desktop`（桌面全量）、`phone`（375×667 触控）、`tablet`（768×1024 触控），后两者跑 `tests/responsive.spec.ts`，覆盖横向溢出、触控尺寸、hover 依赖、插图滚动、表格卡片化、移动端目录抽屉等。

## 分享卡片（Open Graph）

每次分享链接时，平台会抓取页面的 `og:image` 作为预览图。本项目为**每个页面预生成专属卡片**（1200×630）：

- 由 
`npm run og` 生成到 `public/og/*.jpg`（25 张，平均约 72 KB，合计 1.8 MB），产物已提交仓库；
- 卡片内容：章节封面缩略图 + 章节角标 + 大号标题 + 署名，与目录视觉一致；
- 查找顺序：英文页先找 `en-<slug>.jpg` → 复用同名中文卡片 → 都找不到时用 `og/index.jpg`；
- 元信息同时输出 `og:image`、`og:image:secure_url`、`og:image:type`、`og:image:width/height`、`og:image:alt`、`twitter:image`、`twitter:image:alt`。

> **预览不出来的常见原因**（依次排查）：① 平台缓存了旧预览，换个 URL 参数或在平台侧刷新；② 页面刚部署，抓取器访问的还是旧版本；③ 网络到 GitHub Pages 不通畅导致抓取超时——这也是把卡片压到 ~72 KB、改用 JPEG 的原因；④ 部分国内平台对未备案域名不给富预览，这与页面配置无关。

## 目录配图与插图

- **目录封面**：`src/assets/covers/*.svg` 共 13 张（前置 3 + 八章 + 结语/附录），由 
`npm run covers` 生成，首页目录通过 `src/components/CoverCard.astro` 渲染。
- **章节插图**：正文内联 SVG（第 2/5/6 章），随明暗主题换色；`book_tools/` 的 PDF/DOCX 脚本会跳过 `<svg>` 块，因此不影响纸质交付物。
- **换成位图**：`CoverCard` 的 `cover` 属性既接受 import 进来的资源对象，也接受字符串路径。把图片放进 `public/covers/` 后，在 `src/content/docs/index.mdx` 里把 `cover={cCh1}` 改成 `cover={`${base}covers/ch1.png`}` 即可，组件不用动。

> 现有封面是**矢量插画**（由 AI 直接书写的 SVG 源码），不是扩散模型生成的位图。两者各有取舍：矢量版任意缩放不糊、体积极小（13 张共 19 KB）、随主题适配；位图版观感更"插画"。要换位图按上面的方式替换即可。
...
品牌图片（`og.png`、PNG 图标）由 
`npm run brand` 生成，产物已提交仓库，构建流程不依赖它。
