...
- 由 `npm run og` 生成到 `public/og/*.jpg`（25 张，平均约 72 KB，合计 1.8 MB），产物已提交仓库；
...
- **目录封面**：`src/assets/covers/*.svg` 共 13 张（前置 3 + 八章 + 结语/附录），由 `npm run covers` 生成，首页目录通过 `src/components/CoverCard.astro` 渲染。
